...
class ViewTests(TestCase):
    ...
    def test_ad_create_and_detail(self):
        ...
        response = self.client.post(
            reverse("platform_for_sharing:ad_create"),
            {
    ...
    def test_ad_update_permission(self):
        ...
        response = self.client.get(
            reverse("platform_for_sharing:ad_update", args=[self.ad2.pk])
        )
        self.assertEqual(response.status_code, 403)

        # Редактировать свое объявление
        response = self.client.get(
            reverse("platform_for_sharing:ad_update", args=[self.ad1.pk])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_ad_permission(self):
        self.client.login(username="user1", password="pass")
        response = self.client.get(
            reverse("platform_for_sharing:ad_delete", args=[self.ad1.pk])
        )
        self.assertEqual(response.status_code, 200)

        # Попытка удалить чужое объявление
        response = self.client.get(
            reverse("platform_for_sharing:ad_delete", args=[self.ad2.pk])
        )
        self.assertEqual(response.status_code, 403)

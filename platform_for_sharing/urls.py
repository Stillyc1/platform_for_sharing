...
from platform_for_sharing.views import (AdCreateView, AdDeleteView,
                                        AdDetailView, AdListAPIView,
                                        AdListView, AdUpdateView, AdViewSet,
                                        ExchangeProposalCreateAPIView,
                                        ExchangeProposalCreateView,
                                        ExchangeProposalDetailView,
                                        ExchangeProposalListAPIView,
                                        ExchangeProposalListView,
                                        ExchangeProposalUpdateAPIView,
                                        HomeView)
...
urlpatterns = [
    ...
    path(
        "exchange_create/", ExchangeProposalCreateView.as_view(), name="exchange_create"
    ),
    path(
        "exchange_<int:pk>/",
    ...
    path(
        "exchange_api_create/",
        ExchangeProposalCreateAPIView.as_view(),
        name="exchange_api_create",
    ),
    path(
        "exchange_api_update/<int:pk>/",
        ExchangeProposalUpdateAPIView.as_view(),
        name="exchange_api_update",
    ),
    path(
        "exchange_api_list/",
        ExchangeProposalListAPIView.as_view(),
        name="exchange_api_list",
    ),
] + router.urls
